...
**⏳ SIRADAKİ (İNCELEME + C2 BİTTİ → kalan altyapı) — POST-COMPACT BURADAN DEVAM:**
- **★ BÖLÜM C kalanı:**
  1. **C1 · Gerçek ses motoru:** "▷ Seslendir" şu an tarayıcı Web Speech'e düşüyor → ds6 Dinamik Hibrit Router (tr/kaz Piper ONNX · uzb/uig/sah MMS-TTS · chv/bak eSpeak NG WASM) + FastAPI Smart Router. *(VM gerekir; en büyük lokma.)*
  2. **C3 · Ekosistem HfApi-CRON:** metrik (indirme/yıldız) otomatik tazeleme.
  - *(C2 ✅ yapıldı — yukarıda.)*
