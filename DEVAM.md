...
- **★ SIRADAKİ = `plan/YOL-HARITASI.md` (FAZLI):** Faz 1 (füzyonel ek ince-ayrışması = NW kanonik-allomorf; ses denklikleri DİNAMİK+kanıtlı = kullanıcının (b)'si; harita/soy-ağacı UX) → Faz 2 (TTS + LLM/HF ekosistem bölümü, deepsearch 6 & 7 hazır) → Faz 3 (Çuvaşça "dilin kalbi") → Faz 4 (yatay ölçek + eğitim portalı). **Açık sorular karar günlüğünde.** Deepsearch çıktıları: `arastirma/_nlp_*.txt`, `_morfoloji_plani.txt`.
