...
- **MOBİL TAM UYUMLULUK (canlı):** build.py M1 enjeksiyonu — kabuk elemanlarına class + responsive `<style>` (`@media max-width:860px`, helmet'e). Sol nav → **hamburger off-canvas drawer** (durum `body.kn-nav-open` class'ında, React state'siz; `kn-mbar` slim mobil bar). Section/h1/input ölçek (input 16px iOS-zoom önleme), tablolar `overflow-x:auto`, çok-sütun grid'ler tek sütun (framework inline stili boşluk-normalize ettiğinden `[style*="1fr 300px"]` değer-eşleşmesi). **Kognat radyal graf ekrana sığar** (2-sütun→tek; kullanıcı şikayeti çözüldü). Doğrulama: **10 ekran docW=375 (sıfır yatay taşma)** + 7 sayfa görsel ✓ (Analiz morfem kartları/Paradigma çekim/Harita SVG+zoom/Kalite tablo/Uzaklık) + **masaüstü hiç değişmedi**. Q: açılışta API çağrılmaz (home statik), yalnız Paradigma/Karşılaştır'a girince ya da arayınca uyanır.
