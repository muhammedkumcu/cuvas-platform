...
- **E4 ✅** UI 20 dile (LIVE_LN/LNp/SELBOX/Üreteç GEN_LANGS) + **nominatif bare-retry** (Türkmen yalın İŞARETSİZ `kitap<n><pl>`→kitaplar; diğer 19 `<nom>`). commit a0ab150. **20 dil Analiz+Üreteç+Paradigma'da uçtan uca canlı, Preview doğrulandı.**

**⏳ SIRADAKİ (morfoloji planı devam) — BURADAN DEVAM:**
- **E3** dil-bazlı özellik envanteri (FST'den geçerli hâl/iyelik/zaman; nom probe yapıldı — sah ~8 hâl gibi farklar var) · **E5** round-trip eval (`segment_eval`) 20 dile + fiil.
- **T (dış-gold öncelik, kullanıcı):** **T1** UniMorph DOĞRULUK harness (sources/unimorph var; analiz+üretim vs insan-gold → gerçek doğruluk %; dili yoksa "gold yok") · **T2** UD KAPSAM (recall) · **T3** lexicon büyüklüğü (apertium .dix lemma sayıları) · **T4** standart korpus kapsamı (dil başına eşit boyut — **kaynak: Wikipedia DEĞİL; resmi haber/akademik korpus; indirmeden ÖNCE kaynak araştır+kullanıcıya sun, gerekirse deepsearch promptu**).
- **G (gösterim: ikisi de):** **G1** "Kalite & Kapsam" sayfası (20 dil × kalite/lexicon/round-trip/doğruluk/kapsam + tarih + betik linki) · **G2** sayfa-içi mini rozetler → G1'e link.
