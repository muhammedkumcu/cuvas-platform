...
- **T4 ✅ KORPUS KAPSAMI (12/20 dil):** `flores_coverage.py` (FLORES-200, 1012 cümle, CC-BY-SA) → 10 dil: tat **%94.3** (Cyrillic FLORES → tat çözüldü!), kaz %93, tur %90, bak %87, uig %86; aze %35/tuk %64 (küçük sözlükle tutarlı). `corpus_coverage.py` (HF açık korpus) → **nog %70.6** (Nogai-Unified) + **kjh %28.6** (khakas-monolingual 259K cümle) — **kjh çarpıcı: gerçek korpus var ama FST sözlüğü iskelet [546 isim/15 fiil] → dijital yoksulluk kanıtı**. + **farklı-lemma** (korpus-temelli T3 alt-sınırı). Kalan 8 dil (chv/sah/gag/kaa/alt/krc/kum/tyv): Leipzig bu ortamdan **bağlantı-bloke** → dürüstçe boş, `_korpus21`'de listeli. commit 95d97b0/40a7ecd/e9f8d61.
...
**★★ MORFOLOJİ ODAĞI (T+G+U+E) TAMAMLANDI.** 20 dil ölçüldü+gösterildi; analiz/üretim/paradigma uçtan uca canlı, kaynaklı, dürüst. **⏳ SIRADAKİ — BURADAN DEVAM:**
- **T4 kalanı** (opsiyonel, ayrı indirme oturumu): chv/sah/gag/kaa/alt/krc/kum/tyv için Leipzig 10K / HF → KAPSAM tamamla (Leipzig bu ortamdan bloke; başka ortam/VPN gerekebilir).
- **GELECEK-PLANLAR'a eklendi (ne+ne zaman):** OOV→CC0 açık veri (T4 Common Voice) · tam-etiket doğruluğu (T5) · apertium'a geri katkı · paper (20 FST birleşik değerlendirme).
