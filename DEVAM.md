...
- **B) Yatay ölçek (★ DEVAM EDİYOR):** deepsearch 11-18 çıktıları geldi (`arastirma/11..18*.pdf`). **B1 ✅ master envanteri** `languages.master.json` (47 dil/lehçe/tarihsel; ds11 + Glottolog çapraz-kontrol; `platform/etl/build_master.py`). **HARİTA ✅ tüm 47 dile açıldı** (gerçek çizim harita `build_map_bg()` v2 — kıyılar/denizler/dağlar; açgözlü etiket-yerleştirme = çakışma çözümü; era stili tarihsel diller; inline kart vitalite+era). **SIRADAKİ:** 12-18 çıkar (pdfplumber, venv PYTHONPATH) → profiller (23→tüm) · kognat/ses-denklikleri (ds17/18) · uzaklık · A1 `COG_CAT`/`COG_CONC` genişlet · ekosistem. ⚠ **pdfplumber tablo çıkarımı** = `arastirma` venv (`/c/Users/.../turkmence-guncelleme/.venv/Lib/site-packages` + Python311).
