...
- **★ SIRADAKİ = `plan/YOL-HARITASI.md` (FAZLI + DEEPSEARCH İHTİYAÇ HARİTASI):**
  ...
  - **`_tts_asr.txt` (6) ✅ → Faz 2.1 YAPILDI** (profillere "Seslendirme (TTS/ASR)" 5. bölümü, `profiles_tts.json`; gerçek ses motoru = ayrı gelecek altyapı fazı).
  - **★ SIRADAKİ (işlenmemiş, çıkarıldı, commit'li):** `_llm_hf_ekosistem.txt` (7 → **Faz 2.2 LLM/HF ekosistem** — kullanıcının "★ büyük" dediği; muhtemelen yeni "Ekosistem" sayfası/bölümü, araştırmacı hub'ı). Sonra **2.7 KAYNAKLAR** (tüm deepsearch sonrası), ardından deepsearch-bağımsız **1.1/1.3/1.4**. + İşlenmemiş düzeltmeler: deepsearch 8 (Sarı Uygurca→G.Sibirya kol etiketi, Kırım Tatarcası geçişken, Salarca areal); deepsearch 9 displayed-dışı diller (gag/crh/uzb/alt…) yatay ölçekte; TTS gerçek-motor entegrasyonu (Piper/MMS/eSpeak WASM + FastAPI hibrit yönlendirme).
