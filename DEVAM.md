...
### ★★★★★ EN GÜNCEL (29 Haz · YAYIN + MOBİL TAM) — PLATFORM CANLI + MOBİL BİTTİ (compact-sonrası İLK BUNU OKU)
**KÖKEN ilk kez yayında + mobil tam uyumlu.** Kullanıcı kararı: ayrı mimari (UI static instant + API Cloud Run sıfıra-inme bedava), güvenlik öncelikli ilk-deploy. Sonra kullanıcının telefon geri bildirimleriyle MOBİL TAM UYUMLULUK (v1→M11). Tree temiz, push'lu.
...
- **GÜVENLİK (canlı doğrulandı):** CORS allowlist (web.app/firebaseapp izinli, evil engellendi ✓) · **rate-limit 600/dk/IP** (60→600 yükseltildi, kullanıcı onayı; Karşılaştır saçılması ~20 istek/aksiyon + paylaşılan IP'ler → 60 düşüktü; slowapi, X-Forwarded-For) · girdi sınırları (kelime≤80/sorgu≤200/gövde≤8KB) · güvenlik başlıkları (nosniff/DENY/no-referrer) · `/docs`+`/openapi` prod'da kapalı (404 ✓) · root-olmayan konteyner · **salt-okunur API**. Hepsi env-güdümlü (`KOKEN_ENV/KOKEN_ALLOWED_ORIGINS/KOKEN_RATE_LIMIT/KOKEN_MAX_BODY`) → dev davranışı (VM) AYNI.
- **MOBİL TAM UYUMLULUK + UI CİLA — BİTTİ (canlı; v1→v6, TÜM kullanıcı telefon-geri-bildirimi işlendi; ayrıntı: GELISTIRME-GUNLUGU devam-34..39):** Yapı: hamburger off-canvas drawer (`body.kn-nav-open`, React state'siz) + `kn-mbar` üst-bar (**tıklanabilir→home `goHome`**, iki-tonlu "**KÖKEN** Türk Dilleri Atlası + Laboratuvarı"). **KÖK GARANTİ: `#content-scroll{overflow-x:hidden}` → sayfa ASLA yana kaymaz** (tablolar kendi içinde scroll) + `zoom:0.92` uniform font küçültme. Tüm 2-sütun/grid mobilde tek-sütun (framework inline stili boşluk-normalize → DEĞER-eşleşmesi: `[style*="1fr 300px"]`/`380px 1fr`/`1.2fr 0.8fr`/`repeat(`). **Kognat: tek-halka→2-3 eşmerkezli halka + GENEL modda 2 AYRI graf** (her biri merkez "ANA TÜRKÇE"+yarı dil, `_mk()` layout fn + cognateNodes2/cognateSplit) + genel "Tümü" kategorisi kaldırıldı→ilk kategori (`_eff`, 28 kelime). **Paradigma temiz 3-sütun tablo** (52→58px HÂL, Kalite tarzı). Analiz/Üreteç/Karşılaştır morfem-kartları orantılı (kelime 30→19px `>span:first-child`) + **aksiyon butonları** (▷Çözümle/Çek/Karşılaştır, turuncu ÜRET stili, mevcut runAnalyze/runParadigm/runCompare'e bağlı). Input satırları `:has()` flex-wrap (input full-width küçücük kalmaz). Harita parmakla **touch-pan** (`atlasTouchStart/Move`+`touch-action:none`). Profil 5-kart, Uzaklık eşit-liste+görünür-scrollbar+"UZAKLIK RADARI" başlık, home label kaldırıldı+üst boşluk azaldı. **Doğrulama: 360px'te 14/14 sayfa SIFIR yatay taşma; masaüstü HİÇ değişmedi (860px-gated).** Q: açılışta API çağrılmaz (home statik), yalnız Paradigma/Karşılaştır'a girince/arayınca uyanır.
...
- **Öbek 2 — MORFOLOJİ-DOĞRULUK TRACK (BİTEN, kullanıcı "emin ol" dedi, kanıtlı):**
  - **#55** Karşılaştır statik≠dinamik: dinamik (/crosslang+/segment) OTORİTE; ek yazımı **gold round-trip 36/37 (%97.3)** (`compare_consistency_check.py`); fix: compare'e API-kelimeyle gelince otomatik runCompare (componentDidUpdate, `_cmpSeed` guard).
  - **#54** Üreteç dile-duyarlı ZAMAN: **kök neden = apertium zaman etiketi+YAPI dilden dile farklı** (tur aorist/şimdiki/gelecek KOPULA ile, /generate'te kişi-çekilmez → tur yalnız past/ifi/cond; chv/aze/tuk/gag 6 tam). `tense_probe.py` → **FEATTENSE 20 dil** (FST-probe) → Üreteç zaman butonları FEATCASE deseninde gate'li + dil-değişince snap. (kjh fiil-gen yok → gate'siz.)
  - **#56** humanizer TAM denetim: `humanizer_audit_api.py` (host-side, VM cov silinmişti) → /generate sistematik üret+/analyze → **33 kapsanmayan etiket** bulundu (tv/iv/err_orth/subst/gpr_*/ger_*/prc_*/px3sg/prog/opt/qst…) → doğru Türkçe glosslarla eklendi (**59→92**), yeniden denetim **0 kapsanmayan**. "Her dil için doğru" = EVET kanıtlı.
- **KALAN: yalnız #57** Harita GeoJSON arka planı — **gerçek GeoJSON sınır verisi = harici İNDİRME gerektirir** (güvenlik kuralı: açık izin); kullanıcı away → indirme onayı bekliyor. build_map_bg zaten gerçek-lat/lon elle-kıyı; plan: Natural Earth 110m public-domain indir → bbox-clip → `_pj` ile projekte → SVG path → land poligonu değiştir (seas/zoom korunur), preview'da hizalama doğrula. **Yetenek değil, izin sınırında.**
- **YENİ BETİKLER (kanıt):** compare_consistency_check.py · tense_probe.py · humanizer_audit_api.py (+ .json raporları). **DERS:** ① analyze≠generate asimetrisi (tur kopula-zamanları üretilemez) → Üreteç dürüstçe gate'ler. ② humanizer denetimi = /generate→/analyze round-trip ile tag-universe topla (cov dosyasına gerek yok). ③ flicker kökü = analyze_all↔segment okuma farkı (#55 ile dinamik-otorite hizalandı).

### ★★★★ (29 Haz gece-2) — SAYFA-SAYFA İNCELEME BÜYÜK BATCH ~BİTTİ
**~33 commit, tree temiz, push'lu.** devam-26/27/28 (GELISTIRME-GUNLUGU) = kullanıcının ~50 maddelik sayfa-sayfa inceleme notlarının **BÜYÜK ÇOĞUNLUĞU işlendi.** **BİTEN tasklar:** #30, #32-39, #41, #42, #44, #46, #47, #50-53 + #31 (5 sayfa kaynakça) + humanizer/Üreteç. **Öne çıkanlar:** tüm bug'lar (humanizer ham-etiket sızıntısı, Analiz arama senkronu, profil renk şeridi [DesignCanvas border-shorthand ezme], paradigma feedback, crosslang+segment SONLU fiil → chv okuduk→вуларӑмӑр) · menü (Öğren çıkar, Hakkında→ana sayfa altına, Harita/Tarih swap) · **#53 master 288 PDF-çıkarım bozukluğu** (curated, fabrikasyonsuz) · Profiller (Codex adı düzelt + EGIDS skala tıkla-aç) · #39 harita (büyük renkli nokta + en-geniş etiket gizle) · **#31 sayfa-altı KAYNAKLAR 5 sayfa** (_psrc helper) · #51 Kaynaklar 55 künye · sayfa temizliği (örnekler/NÖTR-launchpad/turuncu-satır kaldır, **ds18→gerçek kaynak** [KIRMIZI ÇİZGİ]) · Üreteç auto-örnek + **zaman fallback** (gel→geldi).
**KALAN (opsiyonel/track):** #40 (Tarih box-içi kaynak SİL; sayfa-altı zaten var) · #45 (Üreteç kaynakça) · #48 (Uzaklık okuma-doğrula) · #49 (Ekosistem yeni-kaynak) · #43-kalıntı (ağaç-toggle taşı + kalite-özeti; **seslendir → C1 ertelendi**) · #35 flicker · **#57 Harita GeoJSON arka plan** (veri-sourcing, ayrı tur). **★ ORTA-VADE MORFOLOJİ-DOĞRULUK TRACK (kullanıcı sordu, GELECEK-PLANLAR'da):** **#54** Üreteç dile-duyarlı zaman/kişi/iyelik (apertium dilden-dile etiket farkı; fallback kısmî bant) · **#55** Karşılaştır statik≠dinamik segmentasyon tutarlılığı + ek yazımı gold-doğrulama · **#56** humanizer tam korpus denetimi (VM'de cov silinmiş → tekrar çek; `humanizer_audit.py` hazır).
**Yayın (C1/C3 SONRASI):** site **Cloudflare Pages** (statik, bedava) + API **HF Space / Cloud Run** (Docker, hfst çalışır) önünde Cloudflare (rate-limit+SSL+cache). Salt-okunur API → güvenli. Domain gerekmiyor.
**DOKUNULMAZ:** Öğren + Araştırmacı Merkezi (kullanıcı: ileride büyük güncelleme) · **Tarih & Köken UI** (çok beğenildi, asla bozma) · `.dc.html` ELLE düzenlenmez (build.py). **SIRADAKİ:** kullanıcı test/yorum bekle, ya da kalan opsiyonel/track işleri (#51-derin/#47-zaman-çizelgesi/#57 GeoJSON/#54-56).
...
- **T4 ✅ KORPUS KAPSAMI — 20/20 DİL TAMAMLANDI:** `flores_coverage.py` (FLORES-200, 1012 cümle) → 10 dil (tat **%94.3** Cyrillic FLORES, kaz %93, tur %90…; aze %35/tuk %64 küçük sözlük). `corpus_coverage.py` (HF açık / **fineweb-2**) → diğer 10: **chv %88.2** (chuvash_mono, ÇEKİRDEK, 12.4K lemma!), kum %84, sah %79.7 (28.9K lemma), kaa %79.6, tyv %79, nog %70.6, krc %61.5, gag %46, **alt %32.9 (252 lemma!) + kjh %28.6 = en derin dijital yoksulluk** (sözlük iskelet → metni tanıyamıyor; projenin sebebi ölçülmüş). + korpus-temelli farklı-lemma. **★ DERS (kullanıcı haklıydı):** "Leipzig ayrı ortam" demek kaçamaktı → Leipzig GERÇEKTEN bloke (host timeout + VM connection refused, kanıtlı) AMA **HF/fineweb-2** çözümü vardı; düşük-kaynaklı dil korpusları için **fineweb-2** (1000+ dil) + dedicated HF (chuvash_mono/karakalpak/Nogai/khakas). commit 95d97b0→6d4c89f.
- **G1 ✅ ELDEN GEÇİRİLDİ (5 eksen + tanım + tıkla-aç):** lemma/tanıma TANIMI eksen kutusunda; "Bu tabloyu nasıl okumalı?" detaylı; sözlük tıkla-aç `<details>` tam POS dökümü. tat çevirisi DENENDİ (lemma %98/tanıma %31 kirli → sayı koyulmadı, kanıt commit'li `tat_translit_test.py`).
- **G2 ✅** mini kalite rozeti (`QMETA`/`qBadge`) → Analiz/Paradigma/Üreteç/Karşılaştır'da "KALİTE <tier> · round-trip %X ↗ Kalite & Kapsam" aktif-dile duyarlı, tıkla→G1. commit d8c785b.
- **U1 ✅** Üreteç **dile-duyarlı** (`FEATCASE`, `feature_probe.py` FST'den): Çuvaşça acc YOK / Sahaca gen+loc YOK → geçersiz hâl SOLUK (gizlemiyor, dürüstçe işaretliyor). commit d8c785b.
- **U2 ✅** round-trip köprüsü: Üreteç sonucunda "↻ Bu biçimi analiz et" → üretilen biçim aynı dilde Analiz'e (хӗр üret→çöz doğrulandı). commit d8c785b.

**★★ MORFOLOJİ ODAĞI (T+G+U+E) TAMAMLANDI — 20 dil, 5 eksen, KAPSAM 20/20.** Analiz/üretim/paradigma uçtan uca canlı; her metrik kaynaklı+dürüst. **⏳ SIRADAKİ — BURADAN DEVAM:**
- **Kullanıcı siteyi kendi test edip sayfa-sayfa yorum yapacak** → geri bildirimini al, ona göre cila.
- **GELECEK-PLANLAR'da (ne+ne zaman):** OOV→CC0 açık veri (T4 Common Voice) · tam-etiket doğruluğu (T5) · apertium'a geri katkı · paper (20 FST birleşik değerlendirme).
- Sonra **BÖLÜM C** (C1 ses motoru / C3 CRON) → **BÖLÜM D** (eğitim portalı). Kullanıcı onayıyla.
