...
- **★ SIRADAKİ = `plan/YOL-HARITASI.md` (FAZLI + DEEPSEARCH İHTİYAÇ HARİTASI):**
  ...
  - **`_llm_hf_ekosistem.txt` (7) ✅ → Faz 2.2 YAPILDI** (Araştırmacı Merkezi'ne "Dil × yetenek envanteri" matrisi + 8 org, `ecosystem.json`; NÖTR — olgunluk yargısı yok, kullanıcı kararı).
  ...
  - **★ TÜM DEEPSEARCH + 2.x İÇERİK İŞLENDİ (2.1/2.2/2.5/2.6/2.7 ✅). SIRADAKİ (deepsearch-bağımsız):** **1.1** füzyon ek-ayrışması (NW kanonik-allomorf; DİKKATLİ — gramer-doğrulamalı), **1.3** harita/soy-ağacı UX (Türkiye konumu + tıkla-inline kart), **1.4** Hakkında/iletişim sayfası, **3.1** Çuvaşça "Dilin Kalbi" anlatı sayfası. + İşlenmemiş düzeltmeler: deepsearch 8 (Sarı Uygurca→G.Sibirya kol etiketi, Kırım Tatarcası geçişken-etiket, Salarca areal); deepsearch 9 displayed-dışı diller (gag/crh/uzb/alt…) yatay ölçekte; TTS/ekosistem gerçek-motor & HfApi-CRON entegrasyonu (ayrı altyapı fazları).
