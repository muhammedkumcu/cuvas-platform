...
- **★ SIRADAKİ = `plan/YOL-HARITASI.md` (FAZLI + DEEPSEARCH İHTİYAÇ HARİTASI):**
  ...
  - **★ TÜM DEEPSEARCH İŞLENDİ. SIRADAKİ (deepsearch-bağımsız):** **2.7 KAYNAKLAR büyük güncelleme** (yeni veri dosyaları + tüm deepsearch kaynakları kütüğe), **1.1** füzyon ek-ayrışması (NW kanonik-allomorf), **1.3** harita/UX (Türkiye konumu + tıkla-inline), **1.4** Hakkında/iletişim, **3.1** Çuvaşça "Dilin Kalbi". + İşlenmemiş düzeltmeler: deepsearch 8 (Sarı Uygurca→G.Sibirya kol etiketi, Kırım Tatarcası geçişken, Salarca areal); deepsearch 9 displayed-dışı diller (gag/crh/uzb/alt…) yatay ölçekte; TTS/ekosistem gerçek-motor & HfApi-CRON entegrasyonu (ayrı altyapı fazları).
