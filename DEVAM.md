...
- **★ SIRADAKİ = `plan/YOL-HARITASI.md` (FAZLI + DEEPSEARCH İHTİYAÇ HARİTASI):**
  ...
  - **SIRADAKİ (deepsearch-bağımsız KALAN):** (a) **1.1'in kalan kısmı** — `кӗнеки`/`кӗнекинче` px3sp `-и-` yutulması (ayrı dikkatli `segment_eval`-doğrulamalı pas); (b) **deepsearch 8 kol-düzeltmeleri** (Sarı Uygurca→G.Sibirya kol etiketi, Kırım Tatarcası geçişken, Salarca areal meta-etiket). **DEEPSEARCH BEKLEYEN:** ekosistem zenginleştirme (`arastirma/10-*.prompt.md` çıktısı). **YATAY/ALTYAPI (en son):** deepsearch 9 displayed-dışı diller; TTS/ekosistem gerçek-motor & HfApi-CRON; Saha/Şor "Dilin Kalbi" şablonu.
