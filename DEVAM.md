...
- **★ SIRADAKİ = `plan/YOL-HARITASI.md` (FAZLI + DEEPSEARCH İHTİYAÇ HARİTASI):**
  ...
  - **GELMİŞ + İŞLENMİŞ deepsearch çıktıları:** `_nlp_araclari.txt`+`_nlp_envanteri.txt` (5/5b) ✅, `_morfoloji_plani.txt` (5c) ✅, **`_siniflandirma.txt` (8) ✅ → Faz 2.5 kollar açıklayıcı YAPILDI** (Tarih & Köken'e "altı kol" kartı + Bayes soy ağacı; 25 Haz).
  - **★ TÜM DEEPSEARCH GELDİ — HENÜZ İŞLENMEMİŞ (çıkarıldı, commit'li):** `_tts_asr.txt` (6 → **Faz 2.1 TTS**), `_llm_hf_ekosistem.txt` (7 → **Faz 2.2 LLM/HF ekosistem**), `_profil_oguz/kipcak/karluk/sibirya/cuvas_ogur.txt` (9.1-9.5 → **Faz 2.6 derin profiller**: Dil Profilleri zenginleştir + verimizi çapraz-kontrol et). Sıradaki en yüksek değer: Faz 2.6 (profiller) ya da 2.2 (ekosistem). + deepsearch 8'in işlenmemiş düzeltmeleri: Sarı Uygurca→G.Sibirya, Kırım Tatarcası geçişken, Salarca areal etiketi.
