...
- **★ SIRADAKİ = `plan/YOL-HARITASI.md` (FAZLI + DEEPSEARCH İHTİYAÇ HARİTASI):**
  ...
  - **★ HENÜZ İŞLENMEMİŞ (çıkarıldı, commit'li):** `_tts_asr.txt` (6 → **Faz 2.1 TTS**), `_llm_hf_ekosistem.txt` (7 → **Faz 2.2 LLM/HF ekosistem**). Sıradaki sırayla: **2.1 TTS** → **2.2 ekosistem** → **2.7 KAYNAKLAR** (tüm deepsearch sonrası). Sonra deepsearch-bağımsız Faz 1.1/1.3/1.4. + İşlenmemiş düzeltmeler: deepsearch 8 (Sarı Uygurca→G.Sibirya, Kırım Tatarcası geçişken, Salarca areal); deepsearch 9 displayed-dışı diller (gag/crh/uzb/alt…) yatay ölçekte.
