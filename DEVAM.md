...
- **★ SIRADAKİ = `plan/YOL-HARITASI.md` (FAZLI + DEEPSEARCH İHTİYAÇ HARİTASI):**
  ...
  - **★ FAZ 1 (deepsearch-bağımsız):** **1.3 harita inline-kart ✅**, **1.4 Hakkında/iletişim ✅** YAPILDI. **1.1 füzyon ek-ayrışması:** canlı `/segment` araştırması yapıldı (bulgular `MORFOLOJI-DEGERLENDIRME.md`) — tek-tip bölme YANLIŞ; yalnız temiz sınır (pl+hâl, ör. сенче→сен+че) güvenle bölünür, px+hâl portmanteau korunur, `кӗнекине` iyelik-bug'ı var. **Backend morfoloji + "%100 doğru" mandası → ayrı odaklı, `segment_eval`-doğrulamalı oturumda yapılacak** (uydurma riski yok).
  - **SIRADAKİ:** 1.1 güvenli-bölme (doğrulamalı backend pası) · **3.1 Çuvaşça "Dilin Kalbi"** anlatı sayfası · ekosistem zenginleştirme (`arastirma/10-*.prompt.md` çıktısı gelince). + İşlenmemiş: deepsearch 8 kol-düzeltmeleri (Sarı Uygurca→G.Sibirya, Kırım Tatarcası geçişken, Salarca areal); deepsearch 9 displayed-dışı diller; TTS/ekosistem gerçek-motor & HfApi-CRON (altyapı fazları).
