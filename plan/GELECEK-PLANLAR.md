...
- **C4 · YAYIN (compact sonrası C planlarıyla):** site **Cloudflare Pages** (statik dist, bedava). API (apertium FST + hfst, Docker) seçenekleri: **(a) HF Space** — bedava ama **uyku moduna geçer** (ilk istek ~30-60 sn soğuk başlar; canlı analiz/üretim bunu beklemek zorunda kalır → kötü UX). **(b) Google Cloud Run** — istek-bazlı, min-instance=0'da uyur ama soğuk-başlama hızlı; min-instance=1 ile **hep uyanık** (küçük ücret) → API kesintisiz. **(c) Fly.io / Railway** — benzer. **KARAR (öneri):** Cloud Run (min-instance=1 ya da düşük) önünde **Cloudflare** (rate-limit + SSL + cache); salt-okunur API güvenli. HF Space yedek/demoluk. *(Kullanıcı: "HF Space her zaman uyanık olmayabilir → Google kolaysa onu kullan." → Cloud Run tercih.)* Maliyet düşük; Docker imajı zaten hfst içerir.
