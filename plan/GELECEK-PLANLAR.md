...
- **C4 · YAYIN ✅ YAPILDI (29 Haz, devam-33):** **CANLI — UI https://koken-morfoloji.web.app · API https://koken-api-1087019161757.europe-west3.run.app.** Mimari *ayrı* (kullanıcı onayı): **UI Firebase Hosting** (Cloudflare Pages yerine — Google ekosistemi, CLI hazırdı; eşdeğer bedava CDN) + **API Cloud Run** (`koken-morfoloji`/`europe-west3`). **Sıfıra-inme tercih edildi** (min-instance=0, bedava; soğuk ölçüldü **~1.4sn** — HF Space'in 30-60sn'inden çok iyi → min-instance=1 gereksiz; sürekli-sıcak istenirse tek flag). Güvenlik: CORS allowlist + rate-limit 60/dk + girdi sınırları + başlıklar + /docs kapalı + non-root (hepsi canlı doğrulandı). Cloudflare önünde **gerekmedi** (Cloud Run zaten HTTPS+Google edge DDoS); ileride özel alan adı alınırsa eklenebilir. Runbook: `deploy/README.md`. Docker imajı yalnız hfst+`.hfst` modeller (apertium CLI yok). *(Kullanıcı: "ilk API yayınım, dikkatli+güvenli ol, çoğunu sen yap.")*
...
- **★ TÜM DİLLER İÇİN GENEL MORFOLOJİ GÜÇLENDİRME (kullanıcı onayı, 29 Haz):** capability_probe.py 3 dil-tipi gösterdi — ② Oğuz-kopula (Türkçe; kopula-birleştirici ile çözüldü) ve ③ **zayıf prototip** (Altayca 282 kök, Hakasça 15 fiil, Karaçay-Balkar, Tuva — fiil üretimi zayıf). **Plan (sürekli, misyon-merkezli):** (1) her dilde OOV→kök listesi topla (korpus + Common Voice CC0) → apertium **.lexc/.dix sözlüklerini büyüt**; özellikle prototip dillerin **fiil sözlüğü + morfotaktik** boşluklarını kapat. (2) segment_eval/round-trip + capability_probe **hatalarını** apertium'a issue/PR olarak geri ver. (3) kopula-birleştirici desenini gerekiyorsa başka dillere genişlet (probe-gated). **Hedef:** "dijital uçurumu" ölçmekten **kapatmaya** geç — projenin asıl misyonu. **Ne zaman:** OOV ayağı (T4 Common Voice) + paper sonrası sürekli; en kritik 1-2 dilden başla.

### ★ KULLANICI-GÖRÜNÜR MORFOLOJİ DOĞRULUĞU (29 Haz, kullanıcı notu: "kullanıcının gördüğü kısımların doğruluğundan/tutarlılığından emin ol") — task #54/#55/#56
- **#54 · Üreteç dile-duyarlı zaman/kişi/iyelik (orta vade):** Üreteç "Geçmiş" tur'da boş üretiyordu — apertium FST'leri **dilden dile farklı zaman etiketi** kullanır (tur -di=`<ifi>`, kaz/kir şimdiki~`<aor>`, chv geçmiş=`<past>`). **Şimdilik:** past↔ifi, pres↔aor fallback (kısmî bant, çalışıyor). **Kalıcı:** U1'in FEATCASE (hâl envanteri) mantığını feature_probe ile **zaman+kişi+iyelik**'e genişlet; seçenekleri dile göre süz/etiketle. *(Benzer hata diğer dil×zaman×kişi kombinasyonlarında da olabilir → envanter şart.)*
- **#55 · Karşılaştır statik vs dinamik tutarlılığı + ek yazımı:** Landing-bağlı statik "okuduk" (3-morfem вула/рӑ/мӑр) ile canlı crosslang+segment (2-katman) farklı. **Ne:** doğru olanı tek otorite yap (öneri: canlı motor kanonik), statik örneği hizala/kaldır; üretilen eklerin yüzey yazımını insan-gold ile doğrula. **Genel:** tüm kullanıcı-görünür morfolojik-API çıktılarının doğruluk denetimi.
- **#56 · Humanizer tam korpus denetimi:** `humanizer_audit.py` yazıldı (her dilde gerçek kelimeleri çöz → kapsanmayan etiketi raporla). VM'de cov silinmiş → korpus tekrar çek. Kontrollü yollar (Üreteç/Paradigma) garanti kapsanıyor; Analiz'te +24 etiket proaktif eklendi; kalan boşluğu bu denetim kapatır. **Ne zaman:** korpus yeniden çekilince (T4 ortamıyla).
