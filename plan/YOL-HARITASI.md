...
- **Deepsearch 8 kol-düzeltmeleri ✅ YAPILDI (25 Haz):** Veri çapraz-kontrolü — `cognates.json` kol etiketleri ZATEN doğruymuş (CrimeanTatar→Kıpçak, Salar→Oğuz, SarygYugur→**Sibirya** [Karluk değil], Khalaj→Argu); **yanlış atama yoktu.** → Tarih & Köken altı-kol kartına **"Geçişken & sınır diller"** nüans notu eklendi: Kırım Tatarcası (Kıpçak gövde + Oğuz temas, geçişken), Salarca (Oğuz + [areal: Amdo]), Sarı Uygurca (adı yanıltıcı; arkaik Güney Sibirya). (Kognat motoruna izogloss RegEx = ileride opsiyonel.)
